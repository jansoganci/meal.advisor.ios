...
-- Migration for MealAdvisor iOS app - FIXED VERSION
...
-- RLS Policies for favorites (only if they don't exist)
DO $$
BEGIN
...
-- RLS Policies for meal_ratings (only if they don't exist)
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'meal_ratings' AND policyname = 'Users can view own ratings') THEN
        CREATE POLICY "Users can view own ratings" ON meal_ratings
            FOR SELECT USING (auth.uid() = user_id);
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'meal_ratings' AND policyname = 'Users can insert own ratings') THEN
        CREATE POLICY "Users can insert own ratings" ON meal_ratings
            FOR INSERT WITH CHECK (auth.uid() = user_id);
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'meal_ratings' AND policyname = 'Users can update own ratings') THEN
        CREATE POLICY "Users can update own ratings" ON meal_ratings
            FOR UPDATE USING (auth.uid() = user_id);
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'meal_ratings' AND policyname = 'Users can delete own ratings') THEN
...
CREATE INDEX IF NOT EXISTS idx_usage_tracking_device_id ON usage_tracking(device_id);
CREATE INDEX IF NOT EXISTS idx_usage_tracking_date ON usage_tracking(tracking_date DESC);

-- Enable Row Level Security
ALTER TABLE usage_tracking ENABLE ROW LEVEL SECURITY;

-- RLS Policies for usage_tracking (only if they don't exist)
DO $$
BEGIN
...
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'update_favorites_updated_at') THEN
        CREATE TRIGGER update_favorites_updated_at BEFORE UPDATE ON favorites
            FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'update_meal_ratings_updated_at') THEN
